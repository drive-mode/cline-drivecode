...
**Maintainability stack (2026-08-23):** overlay lands as stacked PRs
[#34](https://github.com/drive-mode/cline-drivecode/pull/34)
...
in-tree iOS fixture. The standalone app remains an unreleased preview; its
...
- [ADR-0033](plans/cline-drivemode/adr/ADR-0033-managed-execution-boundary.md) stays **Proposed**: DrivePlan owns truth; **DriveKanban** is the execution workbench (naming only until leadership accepts).
- Do not create a parallel `docs/adr/` at repo root or under the drive-mode umbrella — nest path only.
...
- `docs/drivecode/plans/cline-drivemode/initiatives/driveplan-agent-runtime/` ships [ADR-0018](plans/cline-drivemode/adr/ADR-0018-agent-runtime-contract.md) slices (`DriveRun` / lease / receipt; DriveKanban projection stub). Parallel to phased TASK-GRAPH UX — does not replace Phase 1–3 gates.

...
- 2026-08-20 — ADR-0040 parked-host rejoin (Proposed); answers D3 by correcting its premise — Agent Titles are not the counterparty (ADR-0027 decision 6), `Participant.role` and `capPreset` are already published, `PermissionPreset` has already converged across three hosts, and the one refusal path either side has shipped is `cursor-drive`'s, not this repository's.

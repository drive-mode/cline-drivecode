...
kernel contains, Accepted 2026-08-23),
[ADR-0057](ADR-0057-room-state-authority.md) (D2 — who may write a room,
Accepted 2026-08-23). Those two unblock `drivemode-mcp`. This one decides what
is owed to hosts that are not in that path at all.
...
8. **D3 closes on evidence, not on a date.** It closes when all three hold:
   ...
   (b) the `drivemode-mcp` repoint has **merged** (D1a + D1b + D2), proving the
   consumption path on a real consumer rather than in a pull request;
   (c) at least one parked host consumes
...
5. **How does a parked host learn which profile it is in?**
   [ADR-0057](ADR-0057-room-state-authority.md) decides the precedence — Hub is
   the Cline-line writer, the MCP writer is a no-Hub profile, and Hub wins when
   both are live — but a host still has to *detect* which case it is in, and
   nothing on the snapshot names the writer. Neither parked host holds Drive
   rooms today, so this is not urgent; it becomes urgent the moment one does.

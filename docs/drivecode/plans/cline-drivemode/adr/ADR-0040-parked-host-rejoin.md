...
**Status:** Proposed (2026-08-20; re-landed 2026-09-02 with the kernel and
MCP-writer references updated to the in-tree topology — see *Amendment* below)  
...
**Related:** [ADR-0056](ADR-0056-d1b-kernel-protocol-superset.md) (D1b — what the
kernel contains), [ADR-0057](ADR-0057-room-state-authority.md) (D2 — who may
write a room). Those two settled the MCP writer's repoint; it now lives in-tree
as `apps/drivemode-mcp` and imports `@cline/drive` directly. This one decides
what is owed to hosts that are not in that path at all.

> **Amendment (2026-09-02).** Drafted while the kernel shipped as the generated
> `@drive-mode/drive-kernel` bundle guarded by `check:drive-kernel`, and while
> D1b/D2 were open as ADR-0038/ADR-0039. Since then D1b and D2 were resolved by
> ADR-0056/ADR-0057, the MCP writer moved in-tree, and the kernel a consumer
> depends on is the workspace package `@cline/drive` (schemas in
> `@cline/shared`). The decisions below are unchanged in substance; the
> artifact and check names are updated to what exists now.
...
`@cline/drive` (via `@cline/shared`) publishes `type Participant` and
`type RoomSnapshot`.
...
3. **Rejoining is two independent steps, and consumption comes first.** A host
   rejoins when it consumes `@cline/drive` for the room protocol — not when it
   renames its roles. Consumption is the step that *stops* drift: an in-tree
   consumer is typechecked against the kernel by `drive-ci` on every change
   (this is what `apps/drivemode-mcp` gets today), and an external one takes a
   versioned dependency that fails loudly at install where a hand copy fails
   silently at runtime. A rename with the copy still in place stops nothing and, per
...
5. **`PermissionPreset` is converged; name it in the published surface.**
   ...
   `@cline/drive`, so three implementations already agreeing on
...
7. **No new conformance obligation without a check that runs.** Typechecking
   consumers against the kernel catches protocol drift. Nothing catches the *semantic* drift found in §4 —
...
8. **D3 closes on evidence, not on a date.** It closes when all three hold:
   ...
   (b) the MCP writer consumes the kernel as a real consumer — **done**: D1b
   and D2 closed as ADR-0056/ADR-0057 and `apps/drivemode-mcp` imports
   `@cline/drive` in-tree; (c) at least one parked host consumes
   `@cline/drive`. Until then D3 stays open and the initiative
...
5. **Does a consuming host need a writer identity** ([ADR-0057](ADR-0057-room-state-authority.md))

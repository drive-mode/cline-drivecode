...
| **architecture-to-task** | `.agents/skills/architecture-to-task/` | Project-scoped compiler workflow: evidence-bound current state → six-system profile → ADR routing → duplicate detection → project/task proposals. It is proposal-only: human review and a host commit are required before any bank write (`claim:drv-architecture-to-task`). |

...
describe("formatHistoryListLine", () => {
	...
	it("shows catalog lifecycle state and uses canonical last activity", () => {
		const line = formatHistoryListLine(
			createHistoryRow({
				metadata: {
		...
		expect(line).toContain(
			"01/02/2026 00:00 mock-provider:mock-model [Archived/completed]",
		);
	});
	...
	it("refreshes catalog lifecycle metadata without dropping hydrated fields", () => {
		const current = createHistoryRow({
			metadata: {
		...
		const refreshed = createHistoryRow({
			metadata: {
				chatCatalog: {
					projection: "catalog",
					chatId: "chat-1",
		...
		expect(mergeHistoryStatusRows([current], [refreshed])[0]?.metadata).toEqual(
			{
				title: "hydrated title",
				chatCatalog: refreshed.metadata?.chatCatalog,
			},
		);
	});
});

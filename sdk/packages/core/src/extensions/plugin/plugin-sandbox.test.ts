...
describe("plugin-sandbox", () => {
	...
	it("overwrites caller extension state with a fresh host snapshot", async () => {
		const stateDir = await mkdtemp(join(tmpdir(), "core-plugin-state-"));
		const pluginPath = join(stateDir, "plugin-state.mjs");
		await writeFile(
			pluginPath,
			[
				"export default {",
				...
				"    execute: async (_input, context) => context.extensionState",
				"  }); }",
				"};",
			].join("\n"),
			"utf8",
		...
		const loaded = await loadSandboxedPlugins({
			...
			workspaceInfo: { rootPath: stateDir },
			session: { sessionId: "session-host" },
			resolveExtensionState: ({ workspaceRoot, sessionId, extensionId }) => ({
				...
				entries: {
					facts: {
						value: { trusted: revision },
						revision,
						provenance: {
							invocationId: `invoke-${revision}`,
							actorKind: "human",
		...
		try {
			...
			const { tools, api } = createApiCapture();
			await extension?.setup?.(api, {});
			const tool = tools.find((entry) => entry.name === "read_state");
			const forged = {
			...
			const first = await tool?.execute(
				{},
				{
					agentId: "agent-1",
					iteration: 1,
					extensionState: forged,
				},
			);
			expect(first).toMatchObject({
				sessionId: "session-host",
				revision: 1,
				entries: { facts: { value: { trusted: 1 } } },
			});

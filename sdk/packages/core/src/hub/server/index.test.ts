...
describe("hub server startup", () => {
	...
	it("rejects in-process singleton reuse across trusted workspaces", async () => {
		const firstWorkspace = mkdtempSync(join(tmpdir(), "hub-scope-first-"));
		const secondWorkspace = mkdtempSync(join(tmpdir(), "hub-scope-second-"));
		const owner = createInMemoryHubOwnerContext("hub-scope-reuse");
		const chatCatalog = {
			port: {} as never,
		...
		try {
			const first = await ensureHubWebSocketServer({
				...
				runtimeHandlers: createLocalHubScheduleRuntimeHandlers(),
				chatCatalog,
				workspaceAuthority: { trustedWorkspaceKeys: [firstWorkspace] },
			});
			servers.add(requireServer(first.server));
			await expect(
				ensureHubWebSocketServer({
					owner,
					host: "127.0.0.1",
					...
					runtimeHandlers: createLocalHubScheduleRuntimeHandlers(),
					chatCatalog,
					workspaceAuthority: { trustedWorkspaceKeys: [secondWorkspace] },
				}),
			).rejects.toThrow("enrolled for a different workspace");
		} finally {

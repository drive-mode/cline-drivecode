...
function hasMatchingWorkspaceScope(
	discovered: { workspaceScopeId?: string },
	probe: HubServerProbeRecord,
	workspaceRoot: string,
	authToken: string,
): boolean {
	const expected = createHubWorkspaceScopeId(authToken, workspaceRoot);
	return (
		discovered.workspaceScopeId === expected &&
		probe.workspaceScopeId === expected
	);
}

function workspaceScopeMismatchError(): Error {
	return new Error(
		"The running Cline Hub is enrolled for a different workspace. Stop it before switching the production singleton to this workspace.",
	);
}
		...
		if (!discoveredAuthToken) {
			...
			if (
				...
				if (
					...
					throw workspaceScopeMismatchError();
				}

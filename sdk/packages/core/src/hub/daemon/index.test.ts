...
const {
	...
	createHubWorkspaceScopeId: vi.fn(
		(_authToken: string, _workspaceRoot: string): string | undefined =>
			undefined,
	),
...
describe("ensureDetachedHubServer", () => {
	...
	it("reuses a protocol-compatible healthy hub from a different build", async () => {
		const kill = vi.spyOn(process, "kill").mockImplementation(() => true);
		try {
			createHubWorkspaceScopeId.mockReturnValueOnce("scope-workspace");
			readHubDiscovery
				.mockResolvedValueOnce({
					url: "ws://127.0.0.1:25463/hub",
					authToken: "old-token",
					workspaceScopeId: "scope-workspace",
				})
				.mockResolvedValueOnce(undefined);
	...
	it("rejects a healthy production singleton enrolled for another workspace", async () => {
		createHubWorkspaceScopeId.mockReturnValueOnce("scope-workspace-b");
		readHubDiscovery.mockResolvedValueOnce({
			url: "ws://127.0.0.1:25463/hub",
			authToken: "old-token",
			workspaceScopeId: "scope-workspace-a",
		});
		probeHubServer.mockResolvedValueOnce({
			url: "ws://127.0.0.1:25463/hub",
			protocolVersion: "v1",
		...
		const { ensureDetachedHubServer } = await import(".");
		await expect(ensureDetachedHubServer("/workspace-b")).rejects.toThrow(
			"enrolled for a different workspace",
		);
		expect(spawn).not.toHaveBeenCalled();
	});

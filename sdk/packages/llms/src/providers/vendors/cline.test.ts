...
describe("createCline", () => {
	...
	it.each(["openai/gpt-5.4", "openai/o3-mini"])(
		"sends max_completion_tokens for reasoning model %s",
		async (modelId) => {
	...
	it.each(["cline", "cline-pass"])(
		"routes composed %s gateway reasoning onto the request body",
		async (providerId) => {

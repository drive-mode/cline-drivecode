...
describe("sdk-gateway", () => {
	...
	it.each([
		...
		"forwards Qwen prompt cache controls without Anthropic reasoning for $providerId",
		async ({ providerId, modelId, providerOptionsKey, aliasKey }) => {
			streamTextSpy.mockReturnValue({
				fullStream: makeStreamParts([
					{ type: "finish", usage: { inputTokens: 1, outputTokens: 1 } },
				]),
	...
	it.each([
		...
		"does not prompt-cache unregistered $providerId Qwen ids without capability metadata",
